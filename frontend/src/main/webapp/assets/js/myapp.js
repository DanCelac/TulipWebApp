$(function(){
 ...
 //*******************
   //to tackle the csrf token , adica la logare fara acest cod va da eroare,
 ...
 //for display data in front end page
...
if ($table.length) {
	...
	//***********************end code for display data in front end page  
...
if ($adminCategoryTable.length) {
		...
		$adminCategoryTable.DataTable({
			lengthMenu: [ [ 5, 10, 15, -1 ],
							[ '5 Records', '10 Records', '15 Records', 'ALL' ] ],
					pageLength: 5,
					ajax: {
						url: jsonUrl,	
						dataSrc: ''		//pentru a manipula datele din Json return		
					...
					columns: [ 
						...
						/*	{
						   ...
						   api.$('.switch input[type="checkbox"]').on('change' , function() {							
							var checkbox = $(this);
							var checked = checkbox.prop('checked');
							var dMsg = (checked)? 'You want to activate the Category? ': 
							                      ' You want to de-activate the Category?';
							var value = checkbox.prop('value');
						    bootbox.confirm({
								size: 'medium',
						    	title: 'Category Activation / Deactivation',
						    	message: dMsg,
								callback: function (confirmed) {
									...
									if (confirmed) {
									   ...
									   var activationUrl = window.contextRoot +'/manage/category/'+value+'/activation';
